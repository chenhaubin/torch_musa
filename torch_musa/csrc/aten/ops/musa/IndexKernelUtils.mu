...
#include "torch_musa/csrc/aten/ops/musa/MemoryAccess.muh"

namespace at::native {
template <int Alignment, typename index_t>
__global__ void vectorized_gather_kernel(
    char* out,
    char* inp,
    index_t* idx,
    int num_ind,
  ...
  CUDA_KERNEL_ASSERT(
      ind >= 0 && ind < ind_dim_size &&
      "vectorized gather kernel index out of bounds");
  int32_t off = (blockDim.x * blockIdx.y + threadIdx.x) *
      Alignment; // off is guaranteed to be within int32 limits
  if (off >= slice_size)
    return;
  // TODO(@ai-infra): Here we utilize at::musa::memory::ld_vec
  // remove this file after ptx is supported in mtcc
  auto vec = at::musa::memory::ld_vec<Alignment>(inp + ind * inp_stride + off);
  at::musa::memory::st_vec<Alignment>(
      out + blockIdx.x * (int32_t)out_stride + off,
